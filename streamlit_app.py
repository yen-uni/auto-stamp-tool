...
def process_stamp(img_file, remove_bg, flip_h, flip_v, rotation_angle, opacity):
    ...
    # 2. 自動裁切透明邊界 (關鍵修正：確保輸入的公分是「印章圖案本身」的大小)
    bbox = img.getbbox()
    if bbox:
        img = img.crop(bbox)
...
# --- 網頁主標題 ---
st.title("📄 環久國際機構-蓋章小工具V7.2極速版")
st.markdown("只需上傳 PDF 與印章，**直接拖曳紅框「左上角」到蓋章位置**，右側可調整真實尺寸。")
...
if pdf_file and stamp_file:
    ...
    # 側邊欄：尺寸設定 (移除座標輸入，改為純尺寸)
    st.sidebar.markdown("---")
    st.sidebar.markdown("**📐 印章實際尺寸 (公分)**")
    stamp_w_cm = st.sidebar.number_input("印章寬度 (公分)", value=3.00, min_value=0.10, max_value=20.00, step=0.01, format="%.2f")
